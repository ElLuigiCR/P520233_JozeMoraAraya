...
namespace P520233_JozeMoraAraya.Formularios
{
    public partial class FrmProductoGestion : Form
    {
        ...
        {
            BtnAgregar_Click(sender, e, MiProductoLocal);
        }

        private void BtnAgregar_Click(object sender, EventArgs e, Logica.Models.Producto miProductoLocal)
        {
        ...
        private void BtnEliminar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (MiProductoLocal.ProductoID > 0)
                {
                    string msg = string.Format("¿Está seguro de activar al producto {0}?", MiProductoLocal.Nombre);

                    DialogResult respuesta = MessageBox.Show(msg, "Confirmación requerida", MessageBoxButtons.YesNo, MessageBoxIcon.Question);

                    if (respuesta == DialogResult.Yes && MiProductoLocal.Activar())
                    {
                        MessageBox.Show("El Producto ha sido activado", "!!!", MessageBoxButtons.OK, MessageBoxIcon.Information);
